        ...
        <div class="section-content">
            ...
            <div class="card">
                ...
                <div class="card-content">
                    ...
                    <form action="{{ route('doctores.create') }}" method="GET">
                        @csrf 
            ...
            @foreach($listaDoctores as $key)
            <div class="card">
                <img src="..\images\user.png" alt="Profile Image">
                <div class="card-content">
                    <h2>{{ $key -> nombre }}</h2>
                    <p>Especialidad: {{ $key -> especialidad }}</p>
                    <p>Teléfono: {{ $key -> telefono }}</p>
                    <p>Correo: {{ $key -> correo }}</p>
                    <form action="{{ route('doctores.edit', $key->id) }}" method="GET">
                        @csrf 
                        @method('EDIT')
                        <button type="submit" id="ver-submit">Ver</button>
                    </form>
                    <form action="{{ route('doctores.destroy', $key->id) }}" method="POST">
                        @csrf 
                        @method('DELETE')
